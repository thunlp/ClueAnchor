...
QA_Clue_template = 'Key clue information: {}\nQuestion: {}\nAnswer:'
QA_COT_Clueanchor_Prompt = """
...
def process_data(data, tokenizer, args):
    processed_data = []
    for item in data:
        raw_input_clueanchor = QA_COT_Clueanchor_Prompt + QA_Clue_template.format(item['clue_extraction'], item['question'])
        token_query = tokenizer([raw_input_clueanchor])
        ...
        token_aug_psg = token_aug_psg.input_ids[0][:args.max_length - SPECIAL_TOKEN_LENGTH - query_length]
        new_psg = tokenizer.decode(token_aug_psg,skip_special_tokens=True)

        raw_input_clueanchor = QA_COT_Clueanchor_Prompt + augment_template.format(new_psg, QA_Clue_template.format(item['clue_extraction'], item['question']))
        augment_input_clueanchor = [{"role": "user", "content": raw_input_clueanchor}]
        augment_input_clueanchor = tokenizer.apply_chat_template(
            augment_input_clueanchor, add_generation_prompt=True, tokenize=False
        )
        item['augment_input_clueanchor'] = augment_input_clueanchor
...
def main(args):
    ...
    for i in tqdm(range(0, len(processed_data), args.batch_size)):
        ...
        batch_prompts_clueanchor = [item["augment_input_clueanchor"] for item in batch]
        batch_responses_clueanchor = batch_generate(llm, batch_prompts_clueanchor, sampling_params)  # 进行批量推理

        for item, response_clueanchor in zip(batch, batch_responses_clueanchor):
            answer = item["answer"]
            if "</think>" in response_clueanchor:
                clueanchor_think_content, clueanchor_answer_content = response_clueanchor.split("</think>", 1)
            else:
                clueanchor_think_content = ""
                clueanchor_answer_content = response_clueanchor
            
            if "<think>" in clueanchor_think_content:
                clueanchor_think_content = clueanchor_think_content.replace("<think>", "").strip()
            
            extracted_clueanchor_content = extract_answer_content(clueanchor_answer_content)
            item["clueanchor_think"] = clueanchor_think_content
            item["clueanchor_answer"] = extracted_clueanchor_content
            clueanchor_score = _acc_score(extracted_clueanchor_content, answer)
            item["clueanchor_score"] = clueanchor_score

            output_data.append(item)
         
    write_jsonl(args.output_path, output_data)
    print(f"ClueAnchored Knowledge Reasoning complete. Output written to: {args.output_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_length', type=int, default=4196, help='Max tokens length for input')
